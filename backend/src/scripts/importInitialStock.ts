...
async function runImport() {
  ...
  for (let i = 0; i < importData.length; i++) {
    ...
    let realPatrimony: string | null = null;
    ...
    if (item.model && item.model.trim()) nameParts.push(item.model.trim());
    if (realPatrimony) nameParts.push(`(Patrimônio: ${realPatrimony})`);
    if (isDefective) nameParts.push('[DEFEITO]');
