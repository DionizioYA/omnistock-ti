...
export class PDFUtil {
   ...
   */
  public static async generateStockReportPdf(products: any[], title = 'Relatório de Controle de Estoque'): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc.fillColor('#1E293B').fontSize(10).text(
          `Total de Produtos: ${totalItems}   |   Total de Unidades em Estoque: ${totalUnits}   |   Valor Total de Compra: R$ ${totalVal.toFixed(2)}`,
          { align: 'left' }
        );
        doc.moveDown(1);
   ...
   */
  public static async generateMovementsPdf(movements: any[]): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        const doc = new PDFDocument({ margin: 40, size: 'A4', layout: 'landscape' });
        const buffers: Buffer[] = [];
  ...
  public static async generateLoansPdf(loans: any[], title = 'Relatório de Equipamentos Emprestados'): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        const doc = new PDFDocument({ margin: 40, size: 'A4', layout: 'landscape' });
        const buffers: Buffer[] = [];
